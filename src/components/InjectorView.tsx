...
function useAIRecommendations(slots: ParkingSlot[]) {
  return useMemo(() => {
    ...
    const longParked = occupied
      .filter(s => s.parkedAt && (now - s.parkedAt) > 2 * 60 * 60 * 1000) // >2hrs
...
export default function InjectorView({ slots, setSlots, setLogs, backendOnline, apiUrl }: InjectorViewProps) {
  ...
  const injectFault = async (type: 'SRAM_FLIP' | 'THERMAL' | 'PWR_DROP', title: string) => {
    ...
    if (backendOnline) {
      // ── Online: call API, Socket.io will handle state update ──
      try {
      ...
      <div className="max-w-6xl mx-auto flex flex-col gap-6">
        ...
        <div className="glass-panel p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4 text-sm font-semibold flex-wrap">
            ...
            <StatChip label="Nodes OK" value={`${slots.filter(s => s.nodeHealth >= 60).length}/${slots.length}`} color="slate" />
          </div>
        </div>

        {/* ── Tab Bar ────────────────────────────────────────── */}
        <div className="flex gap-2">
          {(['ai', 'faults'] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`px-5 py-2 rounded-xl text-sm font-semibold transition-all flex items-center gap-2
                ${activeTab === tab ? 'bg-white text-black' : 'text-zinc-500 hover:text-white hover:bg-white/10 border border-white/5'}`}
            >
              {tab === 'ai' ? <><Brain className="w-4 h-4" /> AI Intelligence</> : <><AlertCircle className="w-4 h-4" /> Fault Injector</>}
            </button>
          ))}
        </div>
        ...
        <AnimatePresence mode="wait">
          {activeTab === 'ai' ? (
            <motion.div key="ai" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
              transition={{ duration: 0.25 }} className="flex flex-col gap-6">
              ...
              <div className="glass-panel p-6">
                <div className="flex items-center gap-3 mb-5">
                  ...
                  <span className="ml-auto text-xs text-zinc-600 flex items-center gap-1"><RefreshCw className="w-3 h-3" /> Updates every 10s</span>
                </div>
                ...
                <div className="glass-panel p-6">
                  ...
                  <div className="grid grid-cols-6 gap-2">
                    {slots.map(slot => {
                      ...
                      return (
                        <div key={slot.id} title={`${slot.id}: ${h}%`}
              ...
              <div className="glass-panel p-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  {[
                    ...
                    return (
                      <div key={item.label} className={`rounded-xl border p-4 flex flex-col gap-2 ${colorMap[item.color]}`}>
            ...
            <motion.div key="faults" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
              transition={{ duration: 0.25 }} className="flex flex-col gap-6">
